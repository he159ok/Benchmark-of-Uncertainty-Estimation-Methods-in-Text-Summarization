...
class chatGPT_Metric_40_Def(GenerationMetric):
    """
    Calculates ChatGPT 4.0 based NLG scores.
    """
    ...
    def __str__(self):
        return f"chatGPT_def_{self.model}_{self.logprobs}_{self.top_logprobs}_{self.task}_{self.view}"
    ...
    def _get_chatGPT_single_socre(self, hyp, ref, aspect):
        ...
        completion = self.client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": 'system',
                 "content": 'You are an analysis agent to response the uncertainty of each generation via an given aspect. There are five different aspects: 1. coherence refers to whether all the sentences in the given generated text form a coherent body; 2. consistency is the factual alignment between the generated text and the original text; 3. fluency represents the quality of individual sentences in the generated text; 4. relevance means whether the summary contains only the important information of the source document; 5. overall includes all above four aspects ',
                    "role": "user",
