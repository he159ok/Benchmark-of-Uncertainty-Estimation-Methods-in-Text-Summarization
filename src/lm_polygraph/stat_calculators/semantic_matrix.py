...
class SemanticMatrixCalculator(StatCalculator):
    ...
    def __call__(
        ...
        for i, pairs in enumerate(batch_pairs):
            ...
            entail_probs = probs[:, ent_id] # the rate of entail prob
            contra_probs = probs[:, contra_id] # the rate of contra prob
            class_preds = probs.argmax(-1) # # the label of [ent, cont, neut]
