...
from src.lm_polygraph.utils2.model import WhiteboxModel
...
def get_embeddings_from_output(
        ...
        if use_averaging:
            if "decoder" in hidden_state:
                try:
                    ...
                    batch_embeddings_decoder = aggregate( # ([42, 7, 4, 1, 1024])
                        agg_decoder_hidden_states, aggregation_method, axis=1
                    )
                    batch_embeddings_decoder = ( # guess there is 7 transformers in the framwork  ([4, 1024])
                        batch_embeddings_decoder.cpu()
                        .detach()
    ...
    return batch_embeddings, batch_embeddings_decoder # due to seq2seq framework, one is for the encoder output embedding, one is for the decoder output embedding
    ...
    def __call__(
        ...
        if model.model_type == "CausalLM":
            return {
                ...
                "embeddings_decoder": embeddings_decoder.cpu().detach().float().numpy(),
