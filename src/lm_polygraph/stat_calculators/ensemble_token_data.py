...
#     get_collect_fn,
# )
from src.lm_polygraph.utils2.token_restoration import (
    get_collect_fn,
)
...
class EnsembleTokenLevelDataCalculator(StatCalculator):
    ...
    def __call__(
        ...
        if "llama" in model_config._name_or_path:
            model_config.is_encoder_decoder = False
        print(f"model_config is {model_config}")
        if "mbart" in model_config._name_or_path: # or "llama" in model_config._name_or_path:
        ...
        if generation_params.get("num_beams") is None and (
            ...
            generation_params["do_sample"] = False # self added
        ...
        print(f" output.keys() is {output.keys()}")
        # print(print(output['sequences_scores']), output['sequences'])
        # print(print(output2['sequences_scores']), output2['sequences'])
        # print(print(output3['sequences_scores']), output3['sequences'])
