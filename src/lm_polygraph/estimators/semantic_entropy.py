...
class SemanticEntropy(Estimator):
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        for i, input_text in enumerate(stats["input_texts"]): # might be ori input text
            for hyp in stats["sample_texts"][i]: # might be the sampling output
                hyps_list[i].append(" ".join([input_text, hyp])) # each element in hyps_list is a pair of (input_text, hyp)
