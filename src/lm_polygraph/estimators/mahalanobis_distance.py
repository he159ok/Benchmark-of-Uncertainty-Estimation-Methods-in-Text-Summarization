...
class MahalanobisDistanceSeq(Estimator):
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        if not self.is_fitted:
            ...
            self.sigma_inv, _ = compute_inv_covariance( # might be used
        ...
        if self.max < dists.max().item():
            self.max = dists.max().item()
            if self.parameters_path is not None:
                torch.save(self.max, f"{self.full_path}/max.pt")
        if self.min > dists.min().item():
            self.min = dists.min().item()
            if self.parameters_path is not None:
                torch.save(self.min, f"{self.full_path}/min.pt")
