...
class RDESeq(Estimator):
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # print(stats[f"train_embeddings_{self.embeddings_type}"].dtype)
        # embeddings = embeddings.float()
        ### (N/A) stats[f"train_embeddings_{self.embeddings_type}"] = stats[f"train_embeddings_{self.embeddings_type}"].float()
        if str(embeddings.dtype) == 'torch.bfloat16':
            embeddings = embeddings.float()
        # if stats[f"train_embeddings_{self.embeddings_type}"].dtype == 'torch.bfloat16':
        #     stats[f"train_embeddings_{self.embeddings_type}"] = stats[f"train_embeddings_{self.embeddings_type}"].float()
