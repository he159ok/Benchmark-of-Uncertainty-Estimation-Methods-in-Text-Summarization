...
class Dataset_Tofu:
    ...
    def __init__(self, x, y, pre_gen, pre_ei, pre_mr, pre_so, pre_re, pre_tme, pre_co, pre_nms, batch_size: int, use_small=False):
        """
        Parameters:
    ...
    def __iter__(self) -> Iterable[Tuple[List[str], List[str]]]:
        ...
        for i in range(0, len(self.x), self.batch_size):
            ...
            yield (self.x[i: i + self.batch_size], self.y[i: i + self.batch_size],
                   self.pre_gen[i: i + self.batch_size], self.pre_ei[i: i + self.batch_size],
                   self.pre_mr[i: i + self.batch_size], self.pre_so[i: i + self.batch_size],
                   self.pre_re[i: i + self.batch_size], self.pre_tme[i: i + self.batch_size],
                   self.pre_co[i: i + self.batch_size], self.pre_nms[i: i + self.batch_size])
    ...
    def select(self, indices: List[int]):
        ...
        self.pre_gen = np.array(self.pre_gen)[indices].tolist()
        self.pre_ei = np.array(self.pre_ei)[indices].tolist()
        self.pre_mr = np.array(self.pre_mr)[indices].tolist()
        self.pre_so = np.array(self.pre_so)[indices].tolist()
        self.pre_re = np.array(self.pre_re)[indices].tolist()
        self.pre_tme = np.array(self.pre_tme)[indices].tolist()
        self.pre_co = np.array(self.pre_co)[indices].tolist()
        self.pre_nms = np.array(self.pre_nms)[indices].tolist()
        return self
    ...
    def from_csv(
        csv_path: str,
        id: str,  # unique id to differentiate different rows.

            source_name="source",
            reference_name="reference",
            summ_sent_name="summ_sent",
            ei_name="Extrinsic Information",
            mr_name="Mis-Referencing",
            so_name="Stating Opinion As Fact",
            re_name="Reasoning Error",
            tme_name="Tense/Modality Error",
            co_name="Contradiction",
            nms_name="Nuanced Meaning Shift",
            tw_name="total_word",
        ...
        **kwargs
        ...
        x = csv[source_name]
        y = csv[reference_name]
        pre_gen = csv[summ_sent_name]
        pre_ei = 1 - (csv[ei_name] * 1.0 / csv[tw_name]).apply(lambda x: max(x, 0.0))
        pre_mr = 1 - (csv[mr_name] * 1.0 / csv[tw_name]).apply(lambda x: max(x, 0.0))
        pre_so = 1 - (csv[so_name] * 1.0 / csv[tw_name]).apply(lambda x: max(x, 0.0))
        pre_re = 1 - (csv[re_name] * 1.0 / csv[tw_name]).apply(lambda x: max(x, 0.0))
        pre_tme = 1 - (csv[tme_name] * 1.0 / csv[tw_name]).apply(lambda x: max(x, 0.0))
        pre_co = 1 - (csv[co_name] * 1.0 / csv[tw_name]).apply(lambda x: max(x, 0.0))
        pre_nms = 1 - (csv[nms_name] * 1.0 / csv[tw_name]).apply(lambda x: max(x, 0.0))
        ...
        return Dataset_Tofu(x, y, pre_gen, pre_ei, pre_mr, pre_so, pre_re, pre_tme, pre_co, pre_nms, batch_size)
    ...
    def from_datasets(
        ...
        **kwargs
        ...
        return Dataset_Tofu(x, y, batch_size)
    ...
    def load(csv_path, *args, **kwargs):
        ...
        if isinstance(csv_path, str) and os.path.isfile(csv_path):
            return Dataset_Tofu.from_csv(csv_path, *args, **kwargs)
        return Dataset_Tofu.from_datasets(csv_path, *args, **kwargs)
